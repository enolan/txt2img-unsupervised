...
@pytest.mark.parametrize("domain_dim", [3, 16])
def test_train_vmf(domain_dim, inject_keys):
    """
...
@pytest.mark.usefixtures("starts_with_progressbar")
@pytest.mark.parametrize("domain_dim", [3, 16])
def test_train_conditional_vmf(domain_dim, inject_keys):
    """
...
def _tsit5_build_endpoints(
    ...
    Returns tuple (x5, x4, k_last, dir5, dir4) where:
    - x5 is the 5th order accurate position
    - x4 is the 4th order accurate position
...
def test_tsit5_step_callback_controls_step_size():
    ...
    _, final_t, _, accepted_max_dt, _ = _tsit5_integrate_core(
        zero_field,
        None,
        None,
        None,
