...
class ImageModel(nn.Module):
    ...
    def setup(self) -> None:
        ...
        # not doing *intersections* they're just taking an average. N.B. the more caps we condition
        # on the more the model learns about the CLIP embeddings of the examples at train time. One
        # cap may be a very large area, but more allows it to effectively "triangulate". This is
        # good for getting the model to learn as much as possible, but potentially bad since at
        # inference time users will in practice mostly only condition on a single cap. We'll see!
        # Tweaking the distribution of cap radii when we generate the caps might be
        # helpful/necessary.

        # When training, we use precomputed cap-image pairs, generated by gen_training_caps.py
